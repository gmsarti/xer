...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="Xer", description="Nome da aplicação")
    ...
    debug: bool = Field(default=True, description="Modo debug ativado")

    # API Settings
    api_host: str = Field(default="0.0.0.0", description="Host da API")

    api_port: int = Field(default=8000, description="Porta da API", ge=1, le=65535)

    # Database Settings
    database_url: str = Field(
        default="sqlite:///./data/tales_db.sqlite",
        description="URL de conexão com o banco de dados",
    )

    # LangChain/LangGraph Settings
    langchain_api_key: str | None = Field(
        default=None, description="API key para LangChain/LangSmith"
    )

    langchain_project: str = Field(
        default="xer", description="Nome do projeto no LangSmith"
    )

    langchain_tracing_v2: bool = Field(
        default=False, description="Ativar tracing do LangSmith"
    )

    # OpenAI Settings (se for usar)
    openai_api_key: str | None = Field(default=None, description="API key da OpenAI")

    openai_model: str = Field(
        default="gpt-4", description="Modelo da OpenAI a ser usado"
    )
    ...
    log_file: Path | None = Field(default=None, description="Arquivo de log (opcional)")
